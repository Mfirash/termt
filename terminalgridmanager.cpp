...
void clamp_cursor_internal() {
    if (cols > 0) cursor_x = std::max(0, std::min(cursor_x, cols - 1));
    else cursor_x = 0;
    
    if (rows > 0) cursor_y = std::max(0, std::min(cursor_y, rows - 1));
    else cursor_y = 0;
}
...
void handle_csi(char command, const std::string& sequence) {
    ...
    std::string clean_seq = is_private ? sequence.substr(1) : sequence;
    ...
    while (std::getline(ss, item, ';')) {
        try {
            if (!item.empty()) params.push_back(std::stoi(item));
        } catch (...) {}
    }
    ...
    if (is_private) {
        switch (command) {
            case 'h':
                if (params[0] == 25) { cursor_visible = true; }
                break;
            case 'l':
                if (params[0] == 25) { cursor_visible = false; }
                break;
        }
        return; 
    }
    ...
    switch (command) {
        ...
        case 'K': 
            if (cursor_y >= 0 && cursor_y < rows) {
                std::fill(grid[cursor_y].begin() + cursor_x, grid[cursor_y].end(), Cell{ ' ', current_attributes });
            }
            break;
        case 'A': cursor_y -= params[0] ? params[0] : 1; clamp_cursor_internal(); break; 
        case 'B': cursor_y += params[0] ? params[0] : 1; clamp_cursor_internal(); break; 
        case 'C': cursor_x += params[0] ? params[0] : 1; clamp_cursor_internal(); break; 
        case 'D': cursor_x -= params[0] ? params[0] : 1; clamp_cursor_internal(); break; 
    }
}
...
int get_rows() { std::lock_guard<std::mutex> lock(grid_mutex); return rows; }
int get_cols() { std::lock_guard<std::mutex> lock(grid_mutex); return cols; }
int get_cursor_x() { std::lock_guard<std::mutex> lock(grid_mutex); return cursor_x; }
int get_cursor_y() { std::lock_guard<std::mutex> lock(grid_mutex); return cursor_y; }
bool is_cursor_visible() { std::lock_guard<std::mutex> lock(grid_mutex); return cursor_visible; }
...
void hidecursor() { std::lock_guard<std::mutex> lock(grid_mutex); cursor_visible = false; }
void showcursor() { std::lock_guard<std::mutex> lock(grid_mutex); cursor_visible = true; }